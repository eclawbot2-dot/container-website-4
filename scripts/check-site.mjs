...
// External hosts that are verified-safe to link (see lib/config.ts + layout fonts).
...
// Unverified until the flags in lib/config.ts flip — must NOT link out.
const FORBIDDEN = [
  /instagram\.com/i,
  /facebook\.com/i,
  /tiktok\.com/i,
...
// 2. Sitemap points at the canonical host only, and every listed page exists.
if (existsSync(join(OUT, 'sitemap.xml'))) {
  ...
  for (const loc of locs) {
    ...
    const page = rel === '' ? 'index.html' : join(rel, 'index.html');
    if (!existsSync(join(OUT, page))) fail(`sitemap lists ${loc} but out/${page} missing`);
  }
}
...
for (const file of htmlFiles(OUT)) {
  ...
  // Local assets referenced by attributes or inline-style url() must exist.
  for (const m of html.matchAll(
    /(?:(?:href|src)="|url\()(\/images\/[^")]+|\/favicon[^")]*|\/apple-touch-icon[^")]*|\/og-image[^")]*)["\)]/g,
  )) {
    const asset = decodeURIComponent(m[1].split('?')[0]);
    if (!existsSync(join(OUT, asset))) fail(`${rel}: references missing asset ${asset}`);
  }
}
if (pages < 4) fail(`only ${pages} HTML pages exported — expected home + 2 events + 404`);
